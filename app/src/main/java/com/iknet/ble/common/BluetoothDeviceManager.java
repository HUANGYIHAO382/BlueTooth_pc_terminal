...
import com.iknet.ble.event.ScanEvent;
...
public class BluetoothDeviceManager {
    ...
    private IBleCallback receiveCallback = new IBleCallback() {
        ...
        public void onSuccess(final byte[] data, BluetoothGattChannel bluetoothGattInfo, BluetoothLeDevice bluetoothLeDevice) {
            ...
            mListener.onBleData(HexUtil.encodeHexStr(data));
...
//                    .setBluetoothGattChannel(bluetoothGattInfo));
    ...
    private OnBleDataListener mListener;
    public interface OnBleDataListener{
        public void onBleData(String data);
    }

    public void setOnBleDataListener(OnBleDataListener listener){
        mListener = listener;
    }
    /**
     * 操作数据回调
...
//                    .setBluetoothGattChannel(bluetoothGattInfo));
    ...
    public void init(Context context) {
        ...
        //蓝牙相关配置修改
        ViseBle.config()
                .setScanTimeout(8 * 1000)//扫描超时时间，这里设置为永久扫描
