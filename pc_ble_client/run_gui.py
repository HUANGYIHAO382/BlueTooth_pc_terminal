...
def print_pip_missing_help() -> None:
    ...
    print()
    print("请任选一种方式：")
    print("  1) 推荐：先建项目虚拟环境，再用它启动（不污染系统 Python）：")
    print("       .\\setup_venv.ps1")
    print("       .\\.venv\\Scripts\\python.exe run_gui.py")
    print("  2) 用 Windows 启动器指定版本（需已安装 3.10+）：")
    print("       py -3.12 run_gui.py")
    print("  3) 手动指定本机 python.exe 完整路径，例如：")
    print('       "C:\\Users\\你的用户名\\AppData\\Local\\Programs\\Python\\Python312\\python.exe" run_gui.py')
    print("  4) 若必须用当前 Anaconda：打开「Anaconda Prompt」执行：")
